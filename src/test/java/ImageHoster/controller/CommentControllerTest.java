...
public class CommentControllerTest {
    ...
    public void createComment() throws Exception {
        ...
        image.setId(1);
        image.setTitle("WonderWomen");
        image.setDescription("Superhero");

        Mockito.when(imageService.getImage(Mockito.anyInt())).thenReturn(image);

        this.mockMvc.perform(post("/image/1/WonderWomen/comments")
                .param("comment", "First picture")
                .session(session))
                .andExpect(redirectedUrl("/images/1/WonderWomen"));
    }
}
